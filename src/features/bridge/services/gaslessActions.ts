...
export async function startGaslessDepositAction(
  ...
  try {
    ...
    const chainKey = inputs.chain as SupportedChain
    ...
    const quoteConfig: GaslessConfig = {
      ...
      actions: [{
        type: 'contractCall',
        target: chainConfig.tokenMessengerAddress,
        calldata: buildDepositForBurnCalldata(inputs.amount, inputs.destinationAddress, chainConfig),
        value: '0'
      }]
    }
    ...
    await executeDepositForBurn(inputs, chainConfig, userAddress)
